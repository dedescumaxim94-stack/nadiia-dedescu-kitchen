...
1. Execute pending DB migrations in remote:
   ...
   - `2026022504_image_path_storage_keys.sql`
2. Add automated regression tests for admin auth guards, publish flow, ingredient delete conflict, and search ordering.
3. Add duplicate-prevention UX for recipe ingredient selection (select existing or create new).
...
4. Public create flow migration:
   ...
   - legacy compatibility endpoint removed:
     - `POST /api/recipes`
...
3. Validate image-path migration in remote environments:
   - existing full Supabase URLs converted to object keys
   - new writes store keys only
   - render/delete remains backward compatible
